  ...
  seatList: [
    ...
    },
    {
      ...
      danhSachGhe: [
        ...
        { soGhe: "A11", gia: 0, daDat: true, dangChon: false },
        { soGhe: "A12", gia: 0, daDat: true, dangChon: false },
      ],
    },
    {
      hang: "B",
...
const reducer = (state = initialState, action) => {
  switch (action.type) {
    case "UPDATE_STATUS":
      const cloneSeatList = [...state.seatList];
      cloneSeatList.map((rowSeat) => {
        rowSeat.danhSachGhe.map((seat) =>
          seat.soGhe === action.payload
